...
n = length(player)
```
Number of individuals: `r n`

```{r}
x = 1 / n * sum(X)
```

Mean of variable X (points): `r x`

```{r}
y = 1 / n * sum(Y)
```

Mean of variable Y (salary in millions) `r y`

```{r}
varX = 1 / (n - 1) * sum((X - x)^2)
```

Variance of X: `r varX`
...
Standard deviation of X: `r sdX`

```{r}
sdY = sqrt(varY)
```

Standard deviation of Y: `r sdY`

```{r}
covXY = 1 / (n - 1) * sum((X - x) * (Y - y))
```

Covariance between X and Y: `r covXY`

```{r}
corXY = covXY / (sdX * sdY)
```

Correlation between X and Y: `r corXY`
...
Estimated slope of the regression line: `r b1`

```{r}
b0 = y - b1 * x
```

Estimated intercept of the regression line: `r b0`
...
Predicted salary (in millions) for a player who scores 0 points: `r Yhat`
...
Predicted salary (in millions) for a player who scores 100 points: `r Yhat`

```{r}
Yhat = b0 + b1 * 500
```

Predicted salary (in millions) for a player who scores 500 points: `r Yhat`

```{r}
Yhat = b0 + b1 * 1000
```

Predicted salary (in millions) for a player who scores 1000 points: `r Yhat`

```{r}
Yhat = b0 + b1 * 2000
```

Predicted salary (in millions) for a player who scores 2000 points: `r Yhat`
...
The Residual Sum of Squares: `r RSS`

```{r}
TSS = sum((Y - y)^2)
```

The Total Sum of Squares: `r TSS`

```{r}
Rsquared = 1 - (RSS / TSS)
```

The coefficient of determination $R^2$: `r Rsquared`
